...
def evaluate_raw_model(
        model_name: str,
        test_data: List[Dict[str, Any]],  # Potrzebujemy tylko test_data
        label_list: List[str],
        output_dir: str
) -> Dict[str, float]:
    tokenizer = AutoTokenizer.from_pretrained(model_name)
    if not isinstance(tokenizer, PreTrainedTokenizerFast):
        logger.warning(f"Tokenizer {model_name} nie jest 'Fast'.")
    ...
    # Przetwarzamy tylko zbiór testowy, oszczędzamy czas
    test_encodings = convert_to_tokens_and_labels(test_data, tokenizer, label_list)
    test_dataset = NERDataset(test_encodings)

    logger.info("Ładowanie modelu (wersja raw/pretrained)...")
    # UWAGA: To załaduje wytrenowanego BERTa, ale WARSTWA KLASYFIKACJI (głowa) będzie losowa!
    # To jest poprawne zachowanie dla "baseline", bo sprawdzamy model bez fine-tuningu.
    ...
    # Ustawienia tylko do ewaluacji (brak epok, learning rate itp.)
    training_args = TrainingArguments(
        output_dir=output_dir,
        per_device_eval_batch_size=16,
        logging_dir=os.path.join(output_dir, "logs"),
        report_to="none",
        do_train=False,  # Ważne: wyłączamy trening
        do_eval=True  # Ważne: włączamy ewaluację
    )

    trainer = Trainer(
        model=model,
        args=training_args,
        eval_dataset=test_dataset,  # Tylko dataset testowy
        tokenizer=tokenizer,
        data_collator=data_collator,
        compute_metrics=compute_metrics
    )

    logger.info("Rozpoczynanie ewaluacji baseline...")
    results = trainer.evaluate()  # Tylko evaluate(), bez train()
...
def main() -> None:
    ...
    output_base = os.path.join(base_dir, "baseline_outputs")
    ...
    logger.info("Wczytywanie zbioru danych BC5CDR...")
    dataset = load_dataset("bigbio/bc5cdr", name="bc5cdr_bigbio_kb", trust_remote_code=True)

    logger.info("Spłaszczanie danych...")
    flattened_dataset = dataset.map(flatten_data_kb, batched=True, num_proc=1, desc="Flattening")

    # Pobieramy TYLKO zbiór testowy do baseline
    test_data: List[Dict[str, Any]] = list(flattened_dataset["test"])
    logger.info(f"Rozmiar zbioru testowego: {len(test_data)}")

    results_table = {}
    for name, model_name in models.items():
        logger.info(f"Testowanie baseline dla modelu: {name}")
        output_model_dir = os.path.join(output_base, name)

        # Wywołujemy nową funkcję
        results = evaluate_raw_model(
        ...
        final_metrics = {k.replace("eval_", ""): v for k, v in results.items() if k.startswith("eval_")}
        results_table[name] = final_metrics
        logger.info(f"Wyniki baseline dla {name}: {final_metrics}")

    print("\n\n" + "=" * 80)
    print("SZCZEGÓŁOWE PORÓWNANIE WYNIKÓW BEZ FINE-TUNINGU (zbiór testowy BC5CDR)")
    print("=" * 80)
